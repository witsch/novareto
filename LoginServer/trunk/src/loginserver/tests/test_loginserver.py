...
browser_layer = ZopeFanstaticBrowserLayer(loginserver.tests)
...
class MyFunctionalTestCase(unittest.TestCase):
    ...
    def test_foo(self):
        index = queryMultiAdapter((Loginserver(), TestRequest()), name='index')
        self.assertNotEqual(index, None)

        # There is no view called 'index2'
        index2 = queryMultiAdapter((Loginserver(), TestRequest()), name='index2')
