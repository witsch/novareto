...
def kopf (c):
    ...
    '''
    bcp = '/'.join(__file__.split('/')[:-1])+"/bghwlogo_klein.jpg"
...
def createpdf (filename, daten):
    ...
    c.drawString( 2.5*cm, 23.7*cm, 'BGHW - Medienverwaltung')
    c.drawString( 2.5*cm, 23.3*cm, 'Niebuhrstraße 5')
    c.drawString( 2.5*cm, 22.9*cm, '53113 Bonn')

    c.drawString(16.8*cm, 20.0*cm, datum)

    # Betreff
    c.setFont(schriftartfett, 12)
    text = 'Medienbestellung von ' + daten['VornameName'] + ' bei der BGHW'
