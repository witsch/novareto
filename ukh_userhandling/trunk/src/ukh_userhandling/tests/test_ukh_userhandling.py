...
browser_layer = ZopeFanstaticBrowserLayer(ukh_userhandling.tests)
...
class MyFunctionalTestCase(unittest.TestCase):
    ...
    def test_foo(self):
        index = queryMultiAdapter((Ukh_userhandling(), TestRequest()), name='index')
        self.assertNotEqual(index, None)

        # There is no view called 'index2'
        index2 = queryMultiAdapter((Ukh_userhandling(), TestRequest()), name='index2')
